...
You are an autonomous issue-processing pipeline for the **ShiroGuessr-Android** repository (`Kurogoma4D/ShiroGuessr-Android`).
...
```bash
gh issue list --repo Kurogoma4D/ShiroGuessr-Android --state open --limit 1 -S "sort:created-asc" --json number,title,labels
```
...
Task tool:
  ...
  prompt: "Implement issue #<number> for the ShiroGuessr-Android repository."
```
...
Task tool:
  ...
  prompt: |
    ...
    Review PR #<pr-number> in the Kurogoma4D/ShiroGuessr-Android repository.
...
```bash
gh pr merge <pr-number> --repo Kurogoma4D/ShiroGuessr-Android --squash --delete-branch
```
